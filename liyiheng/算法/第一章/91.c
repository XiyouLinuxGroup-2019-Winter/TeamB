...
// @version      4.6.1
// @namespace    coder_tq
// @description  自动挂机看超星尔雅MOOC，支持视频、音频、文档、图书自动完成，章节测验自动答题提交，支持自动切换任务点、挂机阅读时长、自动登录等，解除各类功能限制，开放自定义参数
// @author       coder_tq
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @match        *://*.nbdlib.cn/*
...
setting.tk_num = setting.tk_num == undefined ? 0 : setting.tk_num;
setting.tk_num = isNaN(setting.tk_num) ? 0 : setting.tk_num;
setting.notice = '公告栏';
GM_xmlhttpRequest({
    method: 'GET',
...
function beforeFind() {
    ...
    setting.div = $(
        ...
        '<div style="font-size: medium;">当前查题接口:' + (setting.tk_num % 2 + 1) + '</div>' +
    ...
    switch (setting.tk_num % 2) {
            ...
            GM_xmlhttpRequest({
                ...
                onload: function (xhr) {
                    if (!setting.loop) {
                        ...
                        var html = xhr.responseText.indexOf('{') ? '请求过于频繁，单击下面的切换题库接口按钮试试吧' : $.parseJSON(xhr.responseText).data;
                        setting.div.children('div:eq(0)').data('html', html).siblings('button:eq(0)').click();
                    } else {
                        setting.div.children('div:eq(0)').html('题库异常，单击下面的切换题库接口按钮试试吧...');
                    }
                },
                ontimeout: function () {
                    setting.loop && setting.div.children('div:eq(0)').html(setting.over + '服务器超时，正在重试...');
        ...
        case 1: {
            GM_xmlhttpRequest({
                ...
                data: 'content=' + encodeURIComponent(question.replace(/^<img src="/, "").replace(/">$/, "")),
                timeout: setting.time,
                onload: function (xhr) {
                    if (!setting.loop) {
                    } else if (xhr.status == 200) {
